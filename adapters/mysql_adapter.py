"""
adapters/mysql_adapter.py — MySQL / MariaDB DatabasePort adapter (Phase 7).
"""
...
class MySQLAdapter(DatabasePort):
    ...
    def validate_dialect(self, sql: str) -> list[str]:
        warnings = []
        if "VARCHAR2" in sql.upper():
            warnings.append("Use VARCHAR in MySQL syntax.")
        return warnings
    ...
    def generate_ddl(self, table: TableMetadata) -> str:
        ...
        pk = f",\n    PRIMARY KEY ({', '.join(f'`{k}`' for k in table.primary_key)})" if table.primary_key else ""
    ...
    def explain(self, sql: str) -> ExplainResult:
        return ExplainResult(success=True, plan_text=f"EXPLAIN {sql}")
